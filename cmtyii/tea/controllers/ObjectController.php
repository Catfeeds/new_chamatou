...
//        $this->Rabc();
//        exit();
        //Yii::$app->response->headers->set('X-Powered-By',time());
        $user = new Users();
        $user->login(['phone'=>'15888888888','password'=>'15888888888']);
        #创建许可

//        var_dump(Yii::$app->authManager->getChildren('admin'));
//        exit();
...
//        $auth->assign($admin, 1);
//        var_dump($action->controller->route);
        if($action->id == 'login')
        {
            return true;
        }

        if(!Yii::$app->user->can((string)$action->controller->route))
        {
